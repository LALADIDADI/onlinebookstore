...
<div class="cBody">
    ...
    <table class="layui-table">
        ...
        if($conn->connect_error){
            ...
            $sql = "select * from orderbooklist where bname like '%$bname%'";
            //调用数据库的方法改变
            ...
            while($row = mysqli_fetch_row($result)){
                ...
                echo '<td>'.$row[2].'</td>';
                echo '<td>'.$row[3].'</td>';
                echo "<td><a class='layui-btn layui-btn-xs' href='get_book_update.php?bnumber=".$row[0]."&bname=".$row[1]."&price=".$row[2]."&amount=".$row[3]."'>修改信息</a></td></tr>";
            }
            if(!mysqli_num_rows($result)){
                $sql2 = "select * from orderbooklist where bnumber = '$bname'";
